...
shot_way_info = {'Amazon_clothing':{'pairs':[(5,3), (5,5), (3,3), (3,2) ], 'Q':10},
                 ...
                 'email':{'pairs':[(5,3), (5,5), (9,3), (9,5)],'Q':5}}

def load_data(dataset_source):
    class_list_train,class_list_valid,class_list_test=json.load(open('./data/{}_class_split.json'.format(dataset_source)))
    if dataset_source == 'email':
        ns = np.load("./data/{}/{}-edge.npy".format(dataset_source, dataset_source))
        ns = ns.astype(np.int32)
        n1s = ns[:,0]
        n2s = ns[:,1]
        node_info = pkl2dic("./data/{}/{}-node.pkl".format(dataset_source, dataset_source))
        ...
        # ns = np.loadtxt("./data/{}/{}_network".format(dataset_source, dataset_source))
        ns = np.load("./data/{}/{}-edge.npy".format(dataset_source, dataset_source))
        ns = ns.astype(np.int32)
        n1s = ns[:,0]
        n2s = ns[:,1]
        ...
        id_class_map = np.load("./data/{}/{}-label.npy".format(dataset_source,dataset_source)).astype(np.int32) #### !!! use int32
        num_nodes = len(set(id_class_map[0]))
        labels = np.zeros((num_nodes,1))
        labels[id_class_map[0], 0] = id_class_map[1]
        # labels = np.reshape(labels, (num_nodes, 1))
        features = np.load(("./data/{}/{}-feats.npy".format(dataset_source, dataset_source)))
        print('nodes num',num_nodes)
        adj = sp.coo_matrix((np.ones(len(n1s)), (n1s, n2s)),
                                shape=(num_nodes, num_nodes))    
        ...
        n1s = []
        n2s = []
        for line in open("./data/{}/{}_network".format(dataset_source,dataset_source)):
            n1, n2 = line.strip().split('\t')
            n1s.append(int(n1))
            n2s.append(int(n2))

        data_train = sio.loadmat("./data/{}/{}_train.mat".format(dataset_source,dataset_source))
        data_test = sio.loadmat("./data/{}/{}_test.mat".format(dataset_source,dataset_source))
        ...
        adj, features, labels, node_names, attr_names, class_names, metadata=load_npz_to_sparse_graph('./data/cora/cora_full.npz')
